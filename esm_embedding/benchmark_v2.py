...
def evaluate_with_readouts(features, fitness, readout_models, n_folds=5, seed=42):
    ...
    for train_idx, test_idx in kf.split(X):
        ...
        if "mlp" in readout_models:
            m = MLPRegressor(hidden_layer_sizes=(128,), activation='relu',
                             alpha=0.001, batch_size=128, max_iter=50,
        ...
        if "lightgbm" in readout_models:
            try:
                ...
                m = LGBMRegressor(n_estimators=60, max_depth=6, learning_rate=0.05, n_jobs=4,
