...
\title[Conditional Layer Invariance in Protein Language Models]{Conditional Layer
Invariance in Protein Language Models: \rev{Position Leakage and Readout Choice
Determine the Apparent Benefit of Multi-Scale Embeddings}}
...
\textbf{Results:} \rev{We benchmarked two 650M-parameter PLMs on the ProteinGym
substitution benchmark using per-layer probing under three feature arms and four
train--test splits, multi-scale pooling, logit lens scoring, layer-wise similarity
analysis, and a position-lookup control. Representational geometry changed
substantially across depth (adjacent-layer cosine $\approx 0.98$), yet for SaProt the
linear CKA between layers 0 and 32 remained 0.81 despite a raw cosine of $-0.01$: the
network rotates its representations while preserving linearly decodable information.
Under the standard random split, all 33 layers again gave nearly indistinguishable
predictions ($\rho \approx 0.60$), but a control regressor given only random position
vectors reproduced this level ($\rho = 0.600$) and collapsed to $0.10$--$0.14$ under
...
\keywords{protein language models, layer selection, mutation effect prediction,
ProteinGym, position leakage, logit lens, CKA}
...
differences. \rev{We report both the artifacts and the leakage-free re-analysis:
...
fitness effects for single and multiple amino acid substitutions. \rev{All 63 datasets
...
\rev{For SaProt, we additionally prepared real Foldseek 3Di structure tokens by
...
\rev{We computed the $33 \times 33$ cosine similarity matrix of per-layer hidden states,
...
\rev{(2) \textbf{equivalence testing}: for each model, arm, and leakage-free split,
...
linearly with layer distance (Pearson $r = -0.50$). \rev{The decay was smooth and
...
\subsection{\rev{Random-Split Layer Invariance Is Largely Position Memorization}}

We first asked \rev{whether the near-flat random-split layer profile of the submitted
...
\rev{Table~\ref{tab:probing} and Figure~\ref{fig:probing}a report the unified
...
\subsection{\rev{Multi-Scale Pooling Shows No Practical Gain Under a Leakage-Free Protocol}}

\rev{We re-ran the pooling benchmark with the unified pipeline (mutant arm, modulo
...
\subsubsection{\rev{The Previously Reported ESM-2 Asymmetry Was a Cross-Protocol Artifact}}

\rev{The large ESM-2 asymmetry in the submitted version (last-layer Ridge
...
\caption{\textbf{\rev{Per-layer probing, multi-scale pooling, and layer distance under
...
\rev{The re-implemented lens (fp32, per-dataset validation against the official
...
random-split supervised benchmarking conditions. \rev{We now examine the two
...
\rev{Together, these boundary conditions refine the central finding: apparent
...
\subsection{\rev{Structure Tokens Help Only Under Leakage-Free Evaluation}}
...
\rev{The central result of this study is conditional rather than absolute. Under
...
\rev{, where position memorization dominates the task}.
...
constructing qualitatively different representations. \rev{The corrected lens
...
layers ($\cos \approx -0.01$) \rev{alongside their high linear CKA (0.81)}
...
\begin{enumerate}
    ...
    \item \textbf{Standard PLMs (ESM-2, ProtBERT, etc.):} \rev{Under leakage-free
...
\rev{either model} in our benchmark \rev{and was matched or exceeded on
...
regression\rev{; 11 of 63 datasets were capped}) may limit sensitivity to
...
against this concern. \rev{Seventh,} our linear probing approach can only detect
...
identify. \rev{Eighth}, position-level data splitting (Section~3.5) shows that
...
($\Delta\rho = 0.01$) is \rev{a reporting convention rather than a
...
\rev{Under leakage-free evaluation, protein language model layers are close to
...
representational geometry. \rev{Multi-scale pooling added no practical benefit
...
\rev{An archived snapshot of the code, per-dataset results, and extracted
